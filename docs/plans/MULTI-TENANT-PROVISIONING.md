...
**The goal:** When Harris Construction signs up on our marketing site, they get `harris.pitbullconstruction.app` within 60 seconds. They land in a setup wizard, configure their company, invite their team, and start working. No manual provisioning. No emails to Josh asking "can you set up my account?"
...
|--------|------|---------------|-------|
| **`pitbullconstruction.app`** | Professional, clear | ~$14/yr | `.app` is Google-backed, HTTPS-only by default, modern SaaS feel |
...
**Recommendation: `pitbullconstruction.app`**
...
- Customers see `harris.pitbullconstruction.app` and immediately know what it is
...
```
Marketing site:     pitbullconstruction.app (or www.)
Customer app:       {slug}.pitbullconstruction.app
API:                api.pitbullconstruction.app
Admin console:      admin.pitbullconstruction.app (internal)
Docs:               docs.pitbullconstruction.app
```

**Examples:**
- Harris Construction → `harris.pitbullconstruction.app`
- Garrison Enterprises → `garrison.pitbullconstruction.app`  
- Demo/sandbox → `demo.pitbullconstruction.app`
...
public string Slug { get; set; }              // "harris" → harris.pitbullconstruction.app
```
...
3. Create wildcard DNS record: `*.pitbullconstruction.app` → Railway load balancer IP/CNAME
...
DNS Records:
  ...
  *.pitbullconstruction.app  CNAME → pitbull-web.up.railway.app
...
│  - support@pitbullconstruction.app (our support access) │
...
│  - Link: https://{slug}.pitbullconstruction.app/setup   │
│  - Includes: password set link (if new user)            │
│  - From: welcome@pitbullconstruction.app                │
└──────────────────┬──────────────────────────────────────┘
                   │
                   ▼
┌─────────────────────────────────────────────────────────┐
...

{
  ...
  "primaryContact": {
    "firstName": "Mike",
    "lastName": "Harris", 
    "email": "mike@harrisconstruction.com",
    "phone": "555-0123"
  },
...
Response 201:
{
  ...
  "slug": "harris",
  "url": "https://harris.pitbullconstruction.app",
  "adminEmail": "mike@harrisconstruction.com",
  "status": "active"
}
```
...
The public marketing site (`pitbullconstruction.app` or a separate marketing domain) has a signup CTA that collects:
...
| **Support** | `support@pitbullconstruction.app` | Support | Pitbull support team access. Can view data, reset passwords, run diagnostics. Limited write access. | Yes (Pitbull staff only, with audit trail) |
...
public class DefaultAccountSeeder
{
    public async Task SeedDefaultAccounts(Guid tenantId, string slug, ContactInfo primaryContact)
    {
        var accounts = new[]
        {
            new { Email = $"recovery+{slug}@pitbullconstruction.app", Role = "Recovery", 
                  FirstName = "Recovery", LastName = "Account", LoginEnabled = false },
            new { Email = "support@pitbullconstruction.app", Role = "Support",
                  FirstName = "Pitbull", LastName = "Support", LoginEnabled = true },
            new { Email = $"system+{slug}@pitbullconstruction.app", Role = "System",
...
{
    From = "noreply@pitbullconstruction.app",
    To = "mike@harrisconstruction.com",
    TemplateAlias = "welcome",
    TemplateModel = new Dictionary<string, object>
    {
        { "company_name", "Harris Construction" },
        { "setup_url", "https://harris.pitbullconstruction.app/setup" },
...
```
From address:     noreply@pitbullconstruction.app
Reply-to:         support@pitbullconstruction.app

DNS Records (Postmark verification):
  TXT  pm-bounces.pitbullconstruction.app → [Postmark DKIM record]
  TXT  _dmarc.pitbullconstruction.app → "v=DMARC1; p=reject; rua=mailto:dmarc@pitbullconstruction.app"
  CNAME pm._domainkey.pitbullconstruction.app → [Postmark DKIM CNAME]
  MX   pitbullconstruction.app → [Postmark inbound, if needed]
  TXT  pitbullconstruction.app → "v=spf1 include:spf.mtasv.net ~all"
```

**All email flows through one verified domain.** No per-tenant sending domains — that's unnecessary complexity. Customers receive email `From: "Pitbull Construction" <noreply@pitbullconstruction.app>` regardless of their subdomain. The email body references their specific environment URL.
...
- [ ] Register domain (`pitbullconstruction.app` or Josh's choice)
...
**Goal:** `{slug}.pitbullconstruction.app` works. Tenants can be created programmatically.
...
1. **Domain choice:** `pitbullconstruction.app` vs `pitbullcloud.com` vs something else? Josh should register it now regardless — domains are cheap and squatters are not.
