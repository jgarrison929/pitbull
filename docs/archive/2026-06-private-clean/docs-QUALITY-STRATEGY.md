# Pitbull Quality Strategy (Historical / Early 2026)

**Created:** 2026-02-01
**Owner:** Quality Architect (AI Agent)
**Last Updated:** 2026-02-01 (largely superseded; see CHANGELOG for delivered quality work)

> **Note:** This is an early planning document. Many "v0.1.0" / P0 items have been addressed or changed in later releases (0.14/0.15+ financials, RBAC, PWA, etc.). Use as historical reference for risk thinking. Current state should be verified against code, recent audits, and CHANGELOG.md.
...
**Risk (historical note):** Early code used interpolated `set_config` for RLS tenant. Current (verified): `set_config('app.current_tenant', ...)` is the **documented required pattern** (CLAUDE.md / AGENTS.md) because it must be a session variable for Postgres RLS policies. TenantId comes from validated JWT claims (trusted), not user headers. No `@p0` param form for this.

Pattern appears in TenantMiddleware, DemoBootstrapper, etc. Not user-controlled input.

**Mitigation already applied in spirit:** Only derive from auth claims. Added SAVEPOINT usage in places for transaction safety. Review raw SQL usages periodically.
...
| # | Issue | Risk Level | Effort |
|---|-------|-----------|--------|
| 14 | ~~N+1 query detection in dev (4.1)~~ | ~~P2~~ | ✅ Done (PR #146) |
