# PR Demo Video Recordings — Design Spec (HISTORICAL)

> **Status:** Infrastructure delivered in 0.15.0
> **Note:** "Playwright PR demo video recording infrastructure" shipped. Historical design for the process. Current implementation may use the pipeline described (or evolved). Useful for understanding how demos were attached to PRs in active dev phase.

**Implemented as of June 2026 (verified):**
- e2e/ dir with playwright.config.ts, @playwright/test, demo-walkthrough.spec.ts.
- recordings/ with .webm video artifacts + screenshots (e.g. auth.setup).
- CHANGELOG 0.15: "**Playwright PR demo video recording** infrastructure (#39df0ff)".
- Pipeline matches Option A/Hybrid: targeted scripts (e.g. for features), video upload artifacts.
- Also used for demo env? (see fixtures/auth).
Infrastructure present and used for PR demos. (Check .github/workflows or package.json scripts + e2e/tests for exact recording flow in CI/PR comments.)
