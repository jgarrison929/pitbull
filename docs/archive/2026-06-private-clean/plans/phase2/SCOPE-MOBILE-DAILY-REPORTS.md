...
- Blob storage backend shipped (S3/MinIO + local via Pitbull.Storage; used for photos/attachments)
- Weather integration (Open-Meteo? per changelog) for daily reports.

**Implementation Status as of June 2026 (grounded):**
- PmDailyReport + PmDailyReportPhoto (with DocumentId, Caption, TakenAt?, GPS?) + WeatherSummary in ProjectManagementEntities + configs. (WeatherCondition enum also present.)
- DailyReportService supports photo create/list via CreateAsync<PmDailyReportPhoto>.
- Mobile daily report page: /daily-reports/mobile/
- CHANGELOG 0.15: "Weather API integration for daily reports", "Delivery ticket OCR", "Blob storage abstraction".
- Camera input pattern for photos possible in browser.
- Offline PWA mentioned.
Core + photos + weather + blob implemented. "Auto-populate weather", photo gallery, compression likely partial. Verify daily report detail UI for photo thumbnails + weather fields on PmDailyReport. (Doc's "no blob yet" is outdated.)
