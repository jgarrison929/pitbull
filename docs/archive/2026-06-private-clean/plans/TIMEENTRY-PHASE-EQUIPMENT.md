# Plan: Time Entry Phase Codes & Equipment Tracking (HISTORICAL)

**Date:** 2026-02-13
**Status:** Implemented (foundational to time tracking / job cost in 0.14-0.15)
**Note:** This plan was executed. TimeEntry supports PhaseId, EquipmentId + EquipmentHours. Crew timecard grid uses it. Cost code auto-assign for labor. See TimeEntry domain, TimeEntryService, crew UI, Equipment entity in Core. Matches delivered "Progress → Schedule → Cost" and equipment in reports.

**Implemented as of June 2026 (verified):**
- TimeEntry (Pitbull.TimeTracking/Domain/TimeEntry.cs): PhaseId?, EquipmentId?, EquipmentHours, CostCodeId (auto-assigned LAB for crew), ProjectId, etc.
- Equipment (Pitbull.Core/Domain/Equipment.cs): Code, Name, Type (Heavy/Light/Vehicles/Tools/Other), HourlyRate, BillingRate, Serial, LicensePlate. Tenant-scoped (explicit comment).
- Services: TimeEntryService auto-assigns "LAB", supports Phase/Equip in create/batch.
- UI/crew: conditional fields, equipmentList in crew-entry.
- Reports: Equipment utilization, labor by phase.
- Seeded Equipment in SeedDataService.
- Phase entity in Projects (for job cost).
Matches design exactly, including tenant vs company scoping rationale. (Also GPS fields added later.) Verify EquipmentController + reports/equipment.
