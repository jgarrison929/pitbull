# Multi-Company Architecture Design (HISTORICAL)

**Author:** Architecture Agent  
**Date:** February 13, 2026  
**Status:** Implemented (multi-company support + demo seeds in 0.15.0)
**Note:** This design was adopted. Tenant > multiple Company (ICompanyScoped). Shared masters vs company transactional. Current reality includes Company switcher, multi-company seeds, RLS/company filters. See Company entity, UserCompanyAccess, middleware, SeedData for Companies 02/03/04. Useful reference.

**Implemented as of June 2026 (verified):**
- Core: Company entity (src/Modules/Pitbull.Core/Domain/Company.cs) with Code, Name, TaxId etc; BaseEntity + TenantId.
- ICompanyScoped interface on transactional entities (TimeEntry, PmPunchListItem, Bid, etc.).
- UserCompanyAccess (Core/Domain/UserCompanyAccess.cs) for per-user access + IsDefault.
- CompanyMiddleware.cs resolves X-Company-Id / JWT / default, sets app.current_company RLS, AccessibleCompanyIds.
- CompanyContext + ICompanyContext.
- Seeds: DemoBootstrapper.cs defines 02=Summit Water Infrastructure, 03=Summit Highway, 04=Summit Electric + parent Summit Builders Group; full data per company.
- UI: CompanySwitcher.tsx + useCompany context; header/side; multi-company demo signup.
- RLS: set_config for company + tenant.
- CHANGELOG 0.15: "Multi-company demo seed data", "company switcher dropdown".
- Fallback logic in middleware for legacy users.
- Most masters (Equipment, CostCode, Employee?) tenant-wide; transactions company-scoped.
Fully matches design. Inter-company accounting partial? Verify in Billing/WIP for due-to etc if implemented. (See also TenantProvisioningService.)
...
2. Backend creates:
   ...
   c. AppUser: admin@example.com, TenantId = tenant.Id
...
Users:
  Demo Contact C01 (Admin) → Access to: ALL companies, default = 01
