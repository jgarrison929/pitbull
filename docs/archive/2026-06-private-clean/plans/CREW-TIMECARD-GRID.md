# Crew Timecard Grid - Design Doc (HISTORICAL)

**Author:** River (with Josh's domain guidance)
**Date:** 2026-02-15 (implemented in time tracking features by 0.15)
**Status:** Implemented - core grid, daily/weekly modes, phase/equipment, auto cost code logic reflected in current TimeEntry + crew UI
**Note:** This doc accurately describes delivered crew timecard functionality (auto labor cost code, phase for man-hours, equipment hrs, config in settings). Reference actual TimeEntryService, crew grid components, TimecardSettings. Useful domain guidance for job cost by phase.

**Implemented as of June 2026 (verified):**
- TimecardSettings (TimecardMode Daily/Weekly, WeeklyEntryMode Simple/Detailed, RequirePhase, RequireEquipment, DefaultProjectId, WeekStartDay) owned on Company; full CRUD via TimecardSettingsController GET/PUT /api/companies/settings/time-tracking.
- TimeEntry entity (src/Modules/Pitbull.TimeTracking/Domain/TimeEntry.cs) includes PhaseId?, EquipmentId?, EquipmentHours, CostCodeId (nullable in create), GPS fields (Latitude etc.), auto-assign logic for "LAB" cost code in TimeEntryService.cs:746 and BatchCreate.
- Crew entry is DEFAULT view: /time-tracking redirects to /time-tracking/crew-entry (see page.tsx).
- Frontend: CrewEntryGrid (no Cost Code column), conditional Phase/Equipment/Equip Hrs, use-timecard-settings.ts, use-crew-entry-form etc. Weekly grids implemented.
- Seed: CostCode "LAB" (Labor, CostType.Labor) + others in SeedDataService.cs; Equipment entity in Core/Domain/Equipment.cs (tenant-scoped) with seeded examples (CAT 320 etc.).
- API: my-crew endpoint, yesterday-crew, BatchCreateTimeEntries support crew flow.
- Verified via code: columns match doc grid (Employee, Phase, Equipment, Equip Hrs, Reg/OT/DT, Total, Notes); auto cost code + phase/equip for man-hours/equip tracking.
- CHANGELOG 0.15 fixes reference crew timecard grid demo data + auto cost code.
All core described features shipped; weekly modes + settings UI present. (Verify latest by `dotnet build` + frontend pages.)
...
**Why Phase matters (grounded in current TimeEntryService + reports):**
- Phase (from Projects module) drives man-hours grouping (see grouping by PhaseId/Phase.Name in service).
- EquipmentHours separate tracking + cost calc.
- Auto labor CostCode logic in TimeEntry creation.
- Used for phase-based job cost reports, WIP integration (0.15 progress/cost foundation).

See TimeEntryConfiguration.cs for PhaseId/EquipmentHours fields + indexes, and service for crew/phase aggregation.
