...
### Lien Waivers
Current enum (LienWaiverType): Conditional, Unconditional, Progress, Final (plus LienWaiverStatus: Requested, Received, Approved, Rejected).
Matches AIA concepts:
- Conditional/Progress — conditional on payment
- Unconditional/Final — full waiver

Status flow: Requested → Received → Approved. Must be received before releasing retention. See LienWaiver entity (Core) and LienWaiverService (Billing).
