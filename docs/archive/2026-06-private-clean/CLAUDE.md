...
## Codebase Stats (as of mid-2026)

Current approximate state (run `dotnet build` and test commands for latest):
- ~95 controllers, 14 domain modules
- 155+ EF Core migrations
- ~253 test .cs files (unit + integration with Testcontainers)
- Substantial frontend (Next.js 16, 100+ pages)
- Active feature work (see CHANGELOG.md for recent additions like financial reports, Punch List, etc.)

Note: This is a learning and prototype project. Many docs in /plans and /specs are aspirational or from earlier phases. Always verify against actual code.
...
│   │   ├── Controllers/               # ~95 REST API controllers
...
│
├── tests/
│   ├── Pitbull.Tests.Unit/            # ~2k+ unit tests (XUnit, in-memory DB) -- see actual test count in code
