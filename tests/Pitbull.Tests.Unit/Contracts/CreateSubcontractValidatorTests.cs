...
public sealed class CreateSubcontractValidatorTests
{
    private readonly CreateSubcontractValidator _validator = new();

    private static CreateSubcontractCommand CreateValidCommand(
        Guid? projectId = null,
    ...
    {
        return new CreateSubcontractCommand(
    ...
    {
        // Zero is not valid - must be greater than zero
    ...
    [Fact]
    public void Validate_WithValidRetainagePercent_ShouldNotHaveError()
    {
        var command = CreateValidCommand(retainagePercent: 10m);
        var result = _validator.TestValidate(command);
        result.ShouldNotHaveValidationErrorFor(x => x.RetainagePercent);
    }

    [Fact]
    public void Validate_WithZeroRetainagePercent_ShouldNotHaveError()
    {
        var command = CreateValidCommand(retainagePercent: 0m);
        var result = _validator.TestValidate(command);
        result.ShouldNotHaveValidationErrorFor(x => x.RetainagePercent);
    }

    [Fact]
    public void Validate_With100RetainagePercent_ShouldNotHaveError()
    {
        var command = CreateValidCommand(retainagePercent: 100m);
        var result = _validator.TestValidate(command);
        result.ShouldNotHaveValidationErrorFor(x => x.RetainagePercent);
    }
    ...
    {
        var command = CreateValidCommand(subcontractorEmail: null);
        var result = _validator.TestValidate(command);
        result.ShouldNotHaveValidationErrorFor(x => x.SubcontractorEmail);
    }
