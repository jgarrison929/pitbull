...
- **Time-entry audit trail 500:** `from`/`to` date filters bound with `Kind=Unspecified`, which Npgsql rejects against the `timestamptz` audit-log column. Dates are now normalized to a UTC day boundary. The same latent pattern exists in the admin audit-log filters (`AdminAuditController`) and should get a follow-up.
- **Digital-twin spatial overlays 500:** the RFI / progress / schedule zone loaders ran concurrently on the shared scoped `DbContext` (`Task.WhenAll`), racing it into "A second operation was started on this context instance". They now run sequentially.
- **Traceability:** the web `ApiError` and 500 error reports now carry the `traceId`/`correlationId` that `ExceptionMiddleware` already returns, so opaque "An unexpected error occurred" responses can be tied back to the server-side exception.
