...
export default function ProjectChangeOrdersPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchSubcontracts = useCallback(async () => {
    ...
    try {
      const result = await api<PagedResult<Subcontract>>(
        `/api/subcontracts?pageSize=100&projectId=${projectId}`
      );
  ...
  }, [projectId, isProjectIdValid, subcontractFilter, statusFilter, search]);
  ...
  if (!isProjectIdValid) {
    return <p className="p-6 text-destructive">Invalid project ID.</p>;
  }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Track scope changes and cost impacts for this project
          </p>
        </div>
        <Button
        ...
        <CardContent>
          {isLoading ? (
            ...
            <EmptyState
              ...
              description="Create your first change order to start tracking scope modifications for this project."
