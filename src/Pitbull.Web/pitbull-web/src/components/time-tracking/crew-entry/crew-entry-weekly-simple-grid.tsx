...
import type { WeeklySimpleEntryData } from "@/types/crew-entry.types";
import type { Equipment, Phase } from "@/lib/types";

interface CrewEntryWeeklySimpleGridProps {
  entries: WeeklySimpleEntryData[];
  equipmentList?: Equipment[];
  phases?: Phase[];
  onUpdateEntry: (
    employeeId: string,
    field: keyof WeeklySimpleEntryData,
    value: string
  ) => void;
}

export function CrewEntryWeeklySimpleGrid({
  entries,
  equipmentList = [],
  phases = [],
  onUpdateEntry,
}: CrewEntryWeeklySimpleGridProps) {
  ...
  function isEntryComplete(entry: WeeklySimpleEntryData): boolean {
    const total =
    ...
    <Card>
      <CardHeader className="pb-4">
        <div className="flex items-center justify-between">
          <CardTitle className="text-lg">Weekly Crew Hours (Totals)</CardTitle>
