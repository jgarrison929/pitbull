...
public class SpatialService : PmServiceBase, ISpatialService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // 2.17.3: batch zone-link queries (no per-zone N+1; GroupBy in SQL).
        // These must run sequentially: they share the scoped PitbullDbContext, which is not
        // thread-safe. Running them via Task.WhenAll raced the DbContext and threw
        // "A second operation was started on this context instance", 500-ing the overlay.
        ...
        System.Diagnostics.Debug.WriteLine(
            OverlayPerfMetrics.FormatFuelLog(projectId, mode, zoneIds.Count, fuelSw.ElapsedMilliseconds));
