    ...
    IWorkflowTransitionService? workflowTransitions = null) : IVendorInvoiceService
{
    ...
    public async Task<Result<VendorInvoiceDto>> UpdateVendorInvoiceAsync(UpdateVendorInvoiceCommand command, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            if (oldStatus.HasValue
                && oldStatus.Value != VendorInvoiceStatus.Approved
                && invoice.Status == VendorInvoiceStatus.Approved)
            {
                await PostAccrualJournalEntryAsync(invoice, cancellationToken);
            }
    ...
    private async Task PostAccrualJournalEntryAsync(VendorInvoice invoice, CancellationToken cancellationToken)
    {
        if (invoice.AccrualJournalEntryId.HasValue)
            return;

        ChartOfAccount? expenseAccount = await db.Set<ChartOfAccount>()
            .AsNoTracking()
            .FirstOrDefaultAsync(a => a.CompanyId == invoice.CompanyId
                                   && a.AccountNumber == "5200"
                                   && a.IsActive
                                   && !a.IsDeleted,
                cancellationToken);

        ChartOfAccount? apAccount = await db.Set<ChartOfAccount>()
            .AsNoTracking()
            .FirstOrDefaultAsync(a => a.CompanyId == invoice.CompanyId
                                   && a.AccountNumber == "2000"
                                   && a.IsActive
                                   && !a.IsDeleted,
                cancellationToken);

        if (expenseAccount is null || apAccount is null)
        {
            logger.LogWarning(
                "Skipping accrual journal entry for vendor invoice {VendorInvoiceId}: GL accounts 5200/2000 not found for company {CompanyId}",
                invoice.Id,
                invoice.CompanyId);
            return;
        }
        ...
        var jeCommand = new CreateJournalEntryCommand(
            ...
            Description: $"Vendor invoice accrual {invoice.InvoiceNumber}",
            SourceModule: "VendorInvoice",
            SourceDocumentId: invoice.Id,
            SourceDocumentRef: invoice.InvoiceNumber,
            ...
            [
                new CreateJournalEntryLineCommand(
                    ...
                    CreditAmount: 0m,
                    Description: $"Expense accrual - {invoice.InvoiceNumber}"
                ),
                new CreateJournalEntryLineCommand(
                    GlAccountId: apAccount.Id,
                    DebitAmount: 0m,
                    CreditAmount: invoice.TotalAmount,
                    Description: $"AP accrual - {invoice.InvoiceNumber}"
                )
            ]
        );
        ...
        var postResult = await journalEntryService.PostJournalEntryAsync(jeResult.Value!.Id, Guid.Empty, cancellationToken);
        ...
        invoice.AccrualJournalEntryId = jeResult.Value.Id;
    }
